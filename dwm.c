...
// TODO: maybe flip this with static xcb_connection_t *conn;
#include "ipc.h"
...
void drawbar(Monitor *m) {
  ...
  x = 0;
  for (i = 0; i < LENGTH(tags); i++) {
    /* do not draw vacant tags (if hidevacanttags is enabled) */
    ...
    /* if occupied ( and highlightactivetags is enabled, draw indicator*/
    if (highlightnonvacanttags && (occ & 1 << i)) {
      drw_rect(drw, x + activetagpad, 0, w - (activetagpad * 2),
               MIN(activetagstroke, 1), 1, 0);
    }
    if (ulineall ||
        m->tagset[m->seltags] &
...
/* helper function for hot-reloading xresources */
/* for (i = 0; i < LENGTH(colors); i++)
